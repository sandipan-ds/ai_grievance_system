"""
DistilBERT fine-tuning for severity classification (4 classes).
...
def train_one_fold(fold_idx, train_X, train_y, val_X, val_y, labels, device, checkpoint_dir, output_dir):
    ...
    print(f"\n{'='*60}")
    print(f"  FOLD {fold_idx} — TRAINING SEVERITY")
    print(f"{'='*60}")
    ...
    # ReduceLR on plateau after 3 epochs (patience=2)
...
def main():
    ...
    parser.add_argument("--data-path", type=str, required=True,
                        help="Path to cv_fold_data_final.joblib")
    ...
    # Determine unique classes (mapping severity labels)
    all_labels_set = set()
    for d in fold_data:
        all_labels_set.update(d["train_severity"])
        all_labels_set.update(d["val_severity"])
    ...
    for fold_idx, d in enumerate(fold_data):
        metrics, true_labels, pred_labels = train_one_fold(
            fold_idx=fold_idx,
            train_X=d["train_X"],
            train_y=d["train_severity"], # Map target feature: severity
            val_X=d["val_X"],
            val_y=d["val_severity"],   # Map target feature: severity
            labels=labels,
            device=device,
    ...
    # Save results
    results_path = output_dir / "results_distilbert_severity.json"
    with open(results_path, "w") as f:
        json.dump(avg_metrics, f, indent=4, default=str)
    print(f"\n✅ Results saved: {results_path}")

    # Save confusion matrix array
    cm_path = output_dir / "confusion_matrix_distilbert_severity.npy"
    np.save(cm_path, cm)
    print(f"✅ Confusion matrix saved: {cm_path}")

    # Save OOF predictions
    oof_path = output_dir / "oof_predictions_severity.joblib"
    ...
    # ── Final Metric Output ───────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"  DISTILBERT SEVERITY — FINAL TRAINING RESULTS")
    print(f"{'='*60}")
    ...
    print(f"\n  Per Severity Level Metrics (Overall OOF):")
