"""
Trial 5: Fine-tuned DeBERTa-v3-base classifier for severity classification (v2).
...
class DebertaV3Classifier(nn.Module):
    def __init__(self, model_name=MODEL_NAME, dropout=0.1, num_classes=5):
        ...
        self.classifier = nn.Linear(self.deberta.config.hidden_size, num_classes) # 768 → 5

    def forward(self, input_ids, attention_mask):
        outputs = self.deberta(input_ids=input_ids, attention_mask=attention_mask)
        # DeBERTa-v3 does not have pooler_output. We pool via the first ([CLS]) token.
...
class SeverityClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Tokenize pair inputs correctly
...
def train_one_fold(fold_idx, train_X, train_reasons, train_labels,
    ...
    print(f"\n{'=' * 70}")
    print(f"  FOLD {fold_idx} — DEBERTA V3 SEVERITY CLASSIFIER")
    print(f"{'=' * 70}")
    ...
    # ── 4. Model ──────────────────────────────────────────────────────────────
    model = DebertaV3Classifier(MODEL_NAME).to(device)
...
def main():
    parser = argparse.ArgumentParser(
        description="Trial 5: Fine-tuned DeBERTa v3 severity classifier (v2)"
    )
    ...
    val_rouge_ls = [m["val_rouge_l"] for m in all_metrics if "val_rouge_l" in m]
    val_meteors = [m["val_meteor"] for m in all_metrics if "val_meteor" in m]
    if val_rouge_ls:
        avg_metrics["avg_val_rouge_l"] = float(np.mean(val_rouge_ls))
        avg_metrics["avg_val_meteor"] = float(np.mean(val_meteors))

    results_path = output_dir / "results_deberta_v3_classifier_v2.json"
    ...
    oof_path = output_dir / "oof_predictions_deberta_v3_classifier_v2.json"
