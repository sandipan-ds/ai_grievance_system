"""
DeBERTa-v3-base fine-tuning for civic agency classification.
...
LEARNING_RATE     = 2e-5
WEIGHT_DECAY      = 0.01
NUM_EPOCHS        = 10  # Standard fine-tuning epochs to prevent overfitting
SEED              = 43
TASK_NAME         = "civic_agency_deberta_v3"
...
def train_one_fold(fold_idx, train_X, train_y, val_X, val_y, labels, device, checkpoint_dir, output_dir):
    """Train DeBERTa v3 for one fold supporting checkpoint resume."""
    print(f"\n{'='*60}")
    print(f"  FOLD {fold_idx} — TRAINING CIVIC AGENCY WITH DEBERTA V3")
    print(f"{'='*60}")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        # Save checkpoint to GCS (write locally first, then copy)
        print(f"    Saving epoch checkpoint...")
        local_ckpt_temp = Path("/tmp") / f"checkpoint_deberta_fold_{fold_idx}.pt"
        torch.save({
            "epoch": epoch,
    ...
    local_comp_temp = Path("/tmp") / f"completed_deberta_fold_{fold_idx}.json"
...
def main():
    ...
    # Save results
    results_path = output_dir / "results_deberta_v3_civic.json"
    with open(results_path, "w") as f:
        json.dump(avg_metrics, f, indent=4, default=str)
    print(f"\n✅ Results saved: {results_path}")

    # Save confusion matrix array
    cm_path = output_dir / "confusion_matrix_deberta_v3_civic.npy"
    np.save(cm_path, cm)
    print(f"✅ Confusion matrix saved: {cm_path}")

    # Save OOF predictions
    oof_path = output_dir / "oof_predictions_civic_deberta_v3.joblib"
    ...
    # ── Final Metric Output ───────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"  DEBERTA V3 CIVIC AGENCY — FINAL TRAINING RESULTS")
    print(f"{'='*60}")
