"""
RoBERTa-base fine-tuning for civic agency classification.
...
Generates accuracy, precision (macro, micro, weighted), recall (macro, micro, weighted),
and F1 (macro, micro, weighted) at every epoch, fold-end, and overall end of training.
"""
import os
import sys
import shutil
...
LEARNING_RATE     = 2e-5
WEIGHT_DECAY      = 0.01
NUM_EPOCHS        = 10  # Reduced for standard fine-tuning epochs to prevent overfitting
SEED              = 43
TASK_NAME         = "civic_agency_roberta"
...
def train_one_fold(fold_idx, train_X, train_y, val_X, val_y, labels, device, checkpoint_dir, output_dir):
    """Train RoBERTa for one fold supporting checkpoint resume."""
    print(f"\n{'='*60}")
    print(f"  FOLD {fold_idx} — TRAINING CIVIC AGENCY WITH ROBERTA")
    print(f"{'='*60}")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        # 9. Compute train metrics (micro, macro, weighted averages)
        ...
        train_prec_macro = precision_score(all_train_labels, all_train_preds, average="macro", zero_division=0)
        train_prec_micro = precision_score(all_train_labels, all_train_preds, average="micro", zero_division=0)
        train_prec_weighted = precision_score(all_train_labels, all_train_preds, average="weighted", zero_division=0)
        
        train_rec_macro = recall_score(all_train_labels, all_train_preds, average="macro", zero_division=0)
        train_rec_micro = recall_score(all_train_labels, all_train_preds, average="micro", zero_division=0)
        train_rec_weighted = recall_score(all_train_labels, all_train_preds, average="weighted", zero_division=0)
        ...
        # 11. Compute validation metrics (micro, macro, weighted averages)
        ...
        val_prec_macro = precision_score(all_labels, all_preds, average="macro", zero_division=0)
        val_prec_micro = precision_score(all_labels, all_preds, average="micro", zero_division=0)
        val_prec_weighted = precision_score(all_labels, all_preds, average="weighted", zero_division=0)
        
        val_rec_macro = recall_score(all_labels, all_preds, average="macro", zero_division=0)
        val_rec_micro = recall_score(all_labels, all_preds, average="micro", zero_division=0)
        val_rec_weighted = recall_score(all_labels, all_preds, average="weighted", zero_division=0)

        current_lr = optimizer.param_groups[0]['lr']
        print(f"\n    Epoch {epoch+1} Summary:")
        print(f"      [Train] Loss: {avg_train_loss:.4f} | Acc: {train_acc:.4f}")
        print(f"              F1  - Macro: {train_f1_macro:.4f} | Micro: {train_f1_micro:.4f} | Weighted: {train_f1_weighted:.4f}")
        print(f"              Prec - Macro: {train_prec_macro:.4f} | Micro: {train_prec_micro:.4f} | Weighted: {train_prec_weighted:.4f}")
        print(f"              Rec  - Macro: {train_rec_macro:.4f} | Micro: {train_rec_micro:.4f} | Weighted: {train_rec_weighted:.4f}")
        print(f"      [Val]   Acc: {val_acc:.4f}")
        print(f"              F1  - Macro: {val_f1_macro:.4f} | Micro: {val_f1_micro:.4f} | Weighted: {val_f1_weighted:.4f}")
        print(f"              Prec - Macro: {val_prec_macro:.4f} | Micro: {val_prec_micro:.4f} | Weighted: {val_prec_weighted:.4f}")
        print(f"              Rec  - Macro: {val_rec_macro:.4f} | Micro: {val_rec_micro:.4f} | Weighted: {val_rec_weighted:.4f}")
        ...
        local_ckpt_temp = Path("/tmp") / f"checkpoint_civic_fold_{fold_idx}.pt"
        torch.save({
            "epoch": epoch,
    ...
    metrics = {
        ...
        "val_precision_macro":    float(precision_score(all_labels, all_preds, average="macro", zero_division=0)),
        "val_precision_micro":    float(precision_score(all_labels, all_preds, average="micro", zero_division=0)),
        "val_precision_weighted": float(precision_score(all_labels, all_preds, average="weighted", zero_division=0)),
        
        "val_recall_macro":       float(recall_score(all_labels, all_preds, average="macro", zero_division=0)),
        "val_recall_micro":       float(recall_score(all_labels, all_preds, average="micro", zero_division=0)),
        "val_recall_weighted":    float(recall_score(all_labels, all_preds, average="weighted", zero_division=0)),
        
        "per_class_report": classification_report(true_labels, pred_labels, output_dict=True, zero_division=0),
    }

    print(f"\n✅ Fold {fold_idx} training finalized.")
    print(f"    Val Accuracy: {metrics['val_accuracy']:.4f}")
    print(f"    Val F1 (Macro | Micro | Weighted): {metrics['val_f1_macro']:.4f} | {metrics['val_f1_micro']:.4f} | {metrics['val_f1_weighted']:.4f}")
    print(f"    Val Precision (Macro | Micro | Weighted): {metrics['val_precision_macro']:.4f} | {metrics['val_precision_micro']:.4f} | {metrics['val_precision_weighted']:.4f}")
    print(f"    Val Recall (Macro | Micro | Weighted): {metrics['val_recall_macro']:.4f} | {metrics['val_recall_micro']:.4f} | {metrics['val_recall_weighted']:.4f}")
    ...
    local_comp_temp = Path("/tmp") / f"completed_civic_fold_{fold_idx}.json"
...
def main():
    ...
    avg_metrics = {
        ...
        "avg_val_precision_macro":    float(np.mean([m["val_precision_macro"] for m in all_metrics])),
        "avg_val_precision_micro":    float(np.mean([m["val_precision_micro"] for m in all_metrics])),
        "avg_val_precision_weighted": float(np.mean([m["val_precision_weighted"] for m in all_metrics])),
        
        "avg_val_recall_macro":       float(np.mean([m["val_recall_macro"] for m in all_metrics])),
        "avg_val_recall_micro":       float(np.mean([m["val_recall_micro"] for m in all_metrics])),
        "avg_val_recall_weighted":    float(np.mean([m["val_recall_weighted"] for m in all_metrics])),
        
        "per_fold_metrics":       all_metrics,
    }

    # Save results
    results_path = output_dir / "results_roberta_civic.json"
    with open(results_path, "w") as f:
        json.dump(avg_metrics, f, indent=4, default=str)
    print(f"\n✅ Results saved: {results_path}")

    # Save confusion matrix array
    cm_path = output_dir / "confusion_matrix_roberta_civic.npy"
    np.save(cm_path, cm)
    print(f"✅ Confusion matrix saved: {cm_path}")

    # Save OOF predictions
    oof_path = output_dir / "oof_predictions_civic_roberta.joblib"
    ...
    # Calculate overall OOF metrics (all averages)
    ...
    oof_prec_macro = precision_score(all_true, all_pred, average="macro", zero_division=0)
    oof_prec_micro = precision_score(all_true, all_pred, average="micro", zero_division=0)
    oof_prec_weighted = precision_score(all_true, all_pred, average="weighted", zero_division=0)
    
    oof_rec_macro = recall_score(all_true, all_pred, average="macro", zero_division=0)
    oof_rec_micro = recall_score(all_true, all_pred, average="micro", zero_division=0)
    oof_rec_weighted = recall_score(all_true, all_pred, average="weighted", zero_division=0)

    # ── Final Metric Output ───────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"  ROBERTA CIVIC AGENCY — FINAL TRAINING RESULTS")
    print(f"{'='*60}")
    print(f"  Average Validation Metrics (across 5 folds):")
    print(f"    Accuracy:     {avg_metrics['avg_val_accuracy']:.4f}")
    print(f"    F1  - Macro: {avg_metrics['avg_val_f1_macro']:.4f} | Micro: {avg_metrics['avg_val_f1_micro']:.4f} | Weighted: {avg_metrics['avg_val_f1_weighted']:.4f}")
    print(f"    Prec - Macro: {avg_metrics['avg_val_precision_macro']:.4f} | Micro: {avg_metrics['avg_val_precision_micro']:.4f} | Weighted: {avg_metrics['avg_val_precision_weighted']:.4f}")
    print(f"    Rec  - Macro: {avg_metrics['avg_val_recall_macro']:.4f} | Micro: {avg_metrics['avg_val_recall_micro']:.4f} | Weighted: {avg_metrics['avg_val_recall_weighted']:.4f}")
    
    print(f"\n  Overall Out-of-Fold (OOF) Metrics:")
    print(f"    Accuracy:     {oof_acc:.4f}")
    print(f"    F1  - Macro: {oof_f1_macro:.4f} | Micro: {oof_f1_micro:.4f} | Weighted: {oof_f1_weighted:.4f}")
    print(f"    Prec - Macro: {oof_prec_macro:.4f} | Micro: {oof_prec_micro:.4f} | Weighted: {oof_prec_weighted:.4f}")
    print(f"    Rec  - Macro: {oof_rec_macro:.4f} | Micro: {oof_rec_micro:.4f} | Weighted: {oof_rec_weighted:.4f}")
