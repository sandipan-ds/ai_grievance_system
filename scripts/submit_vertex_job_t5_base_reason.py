"""
Submit T5-base reason generation training job to Vertex AI Custom Training.
Usage: python scripts/submit_vertex_job_t5_base_reason.py
"""
import subprocess
...
GCS_OUTPUT         = f"{GCS_DATA_DIR}/output_t5_base_reason"
GCS_CHECKPOINT_DIR = f"{GCS_DATA_DIR}/checkpoints_t5_base_reason"
...
JOB_DISPLAY_NAME = f"t5-base-reason-{int(time.time())}"
...
def main():
    ...
    local_script = Path(__file__).resolve().parent / "T5_base_train_reason.py"
    gcs_script_dest = f"{GCS_DATA_DIR}/T5_base_train_reason.py"
    ...
    print(f"\nSubmitting T5-base reason training job: {JOB_DISPLAY_NAME}")
        ...
        worker_pool_specs=[{
            ...
            "container_spec": {
                ...
                "args": [
                    ...
                    "pip install --upgrade joblib scikit-learn pandas numpy sentencepiece nltk && "
                    "pip install 'transformers>=4.40,<4.45' && "
                    "python -u /gcs/ai-grievance-sandi-data/bert_training/severity_v2/T5_base_train_reason.py "
                    f"--data-path /gcs/ai-grievance-sandi-data/bert_training/civic_v2/df_final_nlp_bert_v2.joblib "
                    f"--output-dir /gcs/ai-grievance-sandi-data/bert_training/severity_v2/output_t5_base_reason "
                    f"--checkpoint-dir /gcs/ai-grievance-sandi-data/bert_training/severity_v2/checkpoints_t5_base_reason"
                ],
            },
        }],
