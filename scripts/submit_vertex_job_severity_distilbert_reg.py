"""
Submit Tier B severity regression job (fine-tuned DistilBERT regressor)
to Vertex AI Custom Training.

Usage: python scripts/submit_vertex_job_severity_distilbert_reg.py
"""
import subprocess
...
GCS_OUTPUT         = f"{GCS_DATA_DIR}/output_distilbert_reg"
GCS_CHECKPOINT_DIR = f"{GCS_DATA_DIR}/checkpoints_distilbert_reg"
...
JOB_DISPLAY_NAME = f"distilbert-severity-reg-{int(time.time())}"
...
def main():
    ...
    local_script = Path(__file__).resolve().parent / "distilbert_severity_regressor.py"
    gcs_script_dest = f"{GCS_DATA_DIR}/distilbert_severity_regressor.py"
    ...
    print(f"\nSubmitting Tier B severity regression job: {JOB_DISPLAY_NAME}")
    ...
    print(f"  Model:   distilbert-base-uncased (66M params) + regression head")
        ...
        worker_pool_specs=[{
            ...
            "container_spec": {
                ...
                "args": [
                    ...
                    "pip install --upgrade joblib scikit-learn pandas numpy && "
                    "pip install 'transformers>=4.40,<4.45' && "
                    "python -u /gcs/ai-grievance-sandi-data/bert_training/severity_v2/distilbert_severity_regressor.py "
                    f"--data-path /gcs/ai-grievance-sandi-data/bert_training/civic_v2/df_final_nlp_bert_v2.joblib "
                    f"--output-dir /gcs/ai-grievance-sandi-data/bert_training/severity_v2/output_distilbert_reg "
                    f"--checkpoint-dir /gcs/ai-grievance-sandi-data/bert_training/severity_v2/checkpoints_distilbert_reg"
                ],
            },
        }],
