"""
Download RoBERTa civic agency v2 training results from GCS.
Usage: python scripts/download_results_civic_roberta_v2.py
"""
import subprocess
import sys
import json
...
GCS_OUTPUT   = f"gs://{BUCKET_NAME}/bert_training/civic_v2/roberta_output"

PROJECT_ROOT = Path(__file__).resolve().parent.parent
CHARTS_DIR   = PROJECT_ROOT / "charts_and_graphs"
METRICS_DIR  = PROJECT_ROOT / "metrics_model_civic_bodies" / "roberta_v2"
...
def main():
    ...
    run(f'gcloud storage cp "{GCS_OUTPUT}/results_roberta_civic.json" "{METRICS_DIR}/"')
    print(f"  [OK] Metrics saved.")

    # ── Download confusion matrix ─────────────────────────────────────────────
    print(f"\n[3/4] Downloading confusion matrix to {METRICS_DIR}/")
    run(f'gcloud storage cp "{GCS_OUTPUT}/confusion_matrix_roberta_civic.npy" "{METRICS_DIR}/"')
    print(f"  [OK] Confusion matrix saved.")

    # ── Download OOF predictions ──────────────────────────────────────────────
    print(f"\n[4/4] Downloading OOF predictions to {DATA_DIR}/")
    run(f'gcloud storage cp "{GCS_OUTPUT}/oof_predictions_civic_roberta.joblib" "{DATA_DIR}/oof_predictions_civic_roberta_v2.joblib"')
    print(f"  [OK] OOF predictions saved.")

    # ── Print summary ─────────────────────────────────────────────────────────
    metrics_file = METRICS_DIR / "results_roberta_civic.json"
    if metrics_file.exists():
        with open(metrics_file) as f:
            m = json.load(f)
        print(f"\n{'='*60}")
        print(f"  ROBERTA CIVIC AGENCY v2 — RESULTS SUMMARY")
        print(f"{'='*60}")
    ...
    print(f"\nFiles saved:")
    print(f"  Metrics:    {METRICS_DIR / 'results_roberta_civic.json'}")
    print(f"  CM:         {METRICS_DIR / 'confusion_matrix_roberta_civic.npy'}")
    print(f"  OOF preds:  {DATA_DIR / 'oof_predictions_civic_roberta_v2.joblib'}")
