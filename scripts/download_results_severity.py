"""
Download DistilBERT severity training results from GCS.
Usage: python scripts/download_results_severity.py
"""
import subprocess
import sys
...
GCS_OUTPUT   = f"gs://{BUCKET_NAME}/bert_training/severity/output"
...
def main():
    ...
    run(f'gcloud storage cp "{GCS_OUTPUT}/results_distilbert_severity.json" "{METRICS_DIR}/"')
    print(f"  [OK] Metrics saved.")

    # ── Download confusion matrix ─────────────────────────────────────────────
    print(f"\n[3/4] Downloading confusion matrix to {METRICS_DIR}/")
    run(f'gcloud storage cp "{GCS_OUTPUT}/confusion_matrix_distilbert_severity.npy" "{METRICS_DIR}/"')
    print(f"  [OK] Confusion matrix saved.")

    # ── Download OOF predictions ──────────────────────────────────────────────
    print(f"\n[4/4] Downloading OOF predictions to {DATA_DIR}/")
    run(f'gcloud storage cp "{GCS_OUTPUT}/oof_predictions_severity.joblib" "{DATA_DIR}/"')
    print(f"  [OK] OOF predictions saved.")

    # ── Print summary ─────────────────────────────────────────────────────────
    import json
    metrics_file = METRICS_DIR / "results_distilbert_severity.json"
    if metrics_file.exists():
        with open(metrics_file) as f:
            m = json.load(f)
        print(f"\n{'='*60}")
        print(f"  DISTILBERT SEVERITY — RESULTS SUMMARY")
        print(f"{'='*60}")
    ...
    print(f"\nFiles saved:")
    print(f"  Metrics:    {METRICS_DIR / 'results_distilbert_severity.json'}")
    print(f"  CM:         {METRICS_DIR / 'confusion_matrix_distilbert_severity.npy'}")
    print(f"  OOF preds:  {DATA_DIR / 'oof_predictions_severity.joblib'}")
